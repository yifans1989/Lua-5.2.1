/* Boolean array extend for lua: index model */
...
int 
boolean_array2_string(lua_State *L) {
	BooleanArray *a = CHECK_ARRAY(L);
	lua_pushfstring(L, "BooleanArray(%d)", a->size);
	return 1;
}


static const struct luaL_Reg boolean_array_m[] = {
	{"__newindex", set_boolean_array},
	{"__index", get_boolean_array},
	{"__len", get_boolean_array_size},
	{"__tostring", boolean_array2_string},
	{NULL, NULL}
};
...
create_metatable(lua_State *L)
{
	...
	luaL_setfuncs(L, boolean_array_m, 0);
	lua_pop(L, 1);
}


static const struct luaL_Reg boolean_array_lib_f[] = {
	{"new", new_boolean_array},
	{NULL, NULL}
};


int luaopen_boolean_array_index(lua_State *L) {
	luaL_newlib(L, boolean_array_lib_f);
	create_metatable(L);
	return 1;
}
