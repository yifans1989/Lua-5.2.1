...
static void new_localvarliteral_ (LexState *ls, const char *name, 
                                                size_t sz) {
...
static Proto *addprototype (LexState *ls) {
  ...
  if (fs->np >= f->sizep) {
    int oldsize = f->sizep;
    luaM_growvector(L, f->p, fs->np, f->sizep, Proto *, 
                    MAXARG_Bx, "functions");
...
static void close_func (LexState *ls) {
  ...
  luaM_reallocvector(L, f->upvalues, f->sizeupvalues, 
                     fs->nups, Upvaldesc);
...
static void assignment (LexState *ls, struct LHS_assign *lh, 
                                      int nvars) {
  expdesc e;
...
static void checkrepeated (FuncState *fs, Labellist *ll, 
                           TString *label) {
  int i;
