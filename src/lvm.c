...
static int call_orderTM (lua_State *L, const TValue *p1, 
...
int luaV_equalobj_ (lua_State *L, const TValue *t1, const TValue *t2) {
  ...
  switch (ttype(t1)) {
    ...
    case LUA_TLNGSTR: return luaS_eqlngstr(rawtsvalue(t1), 
                                           rawtsvalue(t2));
    case LUA_TUSERDATA: {
      if (uvalue(t1) == uvalue(t2)) return 1;
      else if (L == NULL) return 0;
      tm = get_equalTM(L, uvalue(t1)->metatable, 
    ...
    case LUA_TTABLE: {
      ...
      tm = get_equalTM(L, hvalue(t1)->metatable, 
...
void luaV_arith (lua_State *L, StkId ra, const TValue *rb,
  ...
  if ((b = luaV_tonumber(rb, &tempb)) != NULL &&
    ...
    lua_Number res = luaO_arith(op - TM_ADD + LUA_OPADD, 
                                nvalue(b), nvalue(c));
    setnvalue(ra, res);
  }
